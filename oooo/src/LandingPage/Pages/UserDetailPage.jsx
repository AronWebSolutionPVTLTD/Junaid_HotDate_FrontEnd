...
const UserDetailPage = () => {
  ...
  const getFavmodel = async () => {
    try {
      const { data } = await axios.get(
        `${BASE_URL}/api/getfavModel/${userInfo._id}`
      );
      setFavModel(data);
    } catch (error) {
      console.log(error);
          ...
          <div className="w-full sm:w-3/5 md:w-4/5 border-b-2 sm:border-b-0 sm:border-r-2 border-orange rounded-2xl">
            <div className="h-full p-5 grid content-between rounded-2xl max-w-3xl gap-y-10">
              <div>
                ...
                <p className="text-lg flex items-center gap-2  mt-1 font-body_font">
                  <span>
                    {userInfo.age}{" "}
                ...
                <div className="grid gap-y-2">
                  ...
                  <p className="text-sm sm:text-lg grid grid-cols-2 gap-3">
                    ...
                    {userInfo.body_hair.map((el, i) => (
                      <span className="block text-right font-body_font" key={i}>
                        {" "}
                        {el},
                      </span>
                    ))}
                  ...
                  <p className="text-sm sm:text-lg grid grid-cols-2 gap-3">
                    <span className="block font-body_font">Speaks:</span>
                    <span className="block text-right font-body_font">
                      {userInfo.speaks}
                    </span>
                  </p>
