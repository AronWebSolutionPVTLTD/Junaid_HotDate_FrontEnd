            ...
            <div className="flex flex-wrap justify-start gap-5 font-light mb-3">
              ...
              <p className="flex flex-wrap items-center text-lg font-light">
                <img
                  ...
                  className="h-4 mr-3"
                />
                <span className="font-body_font">Since {formattedDate}</span>
              </p>
            </div>
            <p className="mb-6 font-body_font text-xl">
              {clubData.description}
            </p>
            <div className="flex flex-wrap justify-between gap-3 items-center">
              <p className="text-lg font-body_font">
                Booking Price : <span>${clubData.booking_price}</span>
              </p>
              {clubData?.ownerId?._id === userInfo._id ? (
                <button className="primary_btn" onClick={handleEditClub}>
                  Edit
                </button>
              ) : (
                <button className="primary_btn">Book Club</button>
              )}
            </div>
          </div>
        </div>
